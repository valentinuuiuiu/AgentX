...
HIVE_MIND_SERVICE_NAMES = ["mcp-hive_mind-layer", "hive_mind-layer", "hive_mind"]
...
DEFI_ANALYZER_SERVICE_NAMES = ["mcp-defi-analyzer", "defi-analyzer-service", "defi-analyzer"]
CRYPTO_TRACKER_SERVICE_NAMES = ["mcp-crypto-tracker", "crypto-tracker-service", "crypto-tracker"]

# Endpoint constants
HIVE_MIND_STATE_ENDPOINT = "/state"
HIVE_MIND_EMOTIONS_ENDPOINT = "/emotions"
...
async def get_mcp_hive_mind_state() -> Optional[Dict[str, Any]]:
    """
    Fetches the state from the MCP HiveMind Layer service.
    """
    log_service_prefix = "MCP HiveMind Layer (State)"
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            service_url = await _get_service_url_from_registry(client, HIVE_MIND_SERVICE_NAMES, log_service_prefix)
            if not service_url: # Error already logged by _get_service_url_from_registry
                return None

            target_url = f"{service_url.rstrip('/')}{HIVE_MIND_STATE_ENDPOINT}"
            logger.info(f"Fetching hive_mind state from {target_url}")
            ...
            if not isinstance(state_data, dict):
                ...
                logger.info("Successfully fetched hive_mind state.")
            return state_data
        ...
        logger.error(f"An unexpected error occurred in get_mcp_hive_mind_state: {e}")
        return None


async def get_mcp_market_emotions() -> Optional[Dict[str, Any]]:
    """
    Fetches the market emotions from the MCP HiveMind Layer service.
    """
    log_service_prefix = "MCP HiveMind Layer (Emotions)"
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            service_url = await _get_service_url_from_registry(client, HIVE_MIND_SERVICE_NAMES, log_service_prefix)
            if not service_url: # Error already logged by _get_service_url_from_registry
            ...
            target_url = f"{service_url.rstrip('/')}{HIVE_MIND_EMOTIONS_ENDPOINT}"
...
async def call_mcp_defi_analyzer(function_name: str, parameters: Dict[str, Any] = None) -> Optional[Dict[str, Any]]:
    """
    Calls the MCP DeFi Analyzer service.
    """
    log_service_prefix = "MCP DeFi Analyzer"
    payload = {"function_name": function_name, "parameters": parameters or {}}
    try:
        async with httpx.AsyncClient(timeout=20.0) as client:
            service_url = await _get_service_url_from_registry(client, DEFI_ANALYZER_SERVICE_NAMES, log_service_prefix)
            if not service_url:
                return None

            target_url = f"{service_url.rstrip('/')}{DEFI_EXECUTE_ENDPOINT}"
            logger.info(f"Calling DeFi Analyzer function '{function_name}' at {target_url}")
            ...
            if response_data.get("success"):
                logger.info(f"Successfully executed DeFi Analyzer function '{function_name}'.")
                return response_data.get("result")
            else:
                logger.warning(f"DeFi Analyzer function '{function_name}' reported failure: {response_data.get('error')}")
                return None

    except Exception as e:
        logger.error(f"Error calling MCP DeFi Analyzer: {e}")
        return None


async def call_mcp_crypto_tracker(function_name: str, parameters: Dict[str, Any] = None) -> Optional[Dict[str, Any]]:
    """
    Calls the MCP Crypto Tracker service.
    """
    log_service_prefix = "MCP Crypto Tracker"
    payload = {"function_name": function_name, "parameters": parameters or {}}
    try:
        async with httpx.AsyncClient(timeout=20.0) as client:
            service_url = await _get_service_url_from_registry(client, CRYPTO_TRACKER_SERVICE_NAMES, log_service_prefix)
            if not service_url:
                return None

            target_url = f"{service_url.rstrip('/')}{DEFI_EXECUTE_ENDPOINT}"
            logger.info(f"Calling Crypto Tracker function '{function_name}' at {target_url}")
            ...
            response_data = response.json()
            if response_data.get("success"):
                logger.info(f"Successfully executed Crypto Tracker function '{function_name}'.")
                return response_data.get("result")
            else:
                logger.warning(f"Crypto Tracker function '{function_name}' reported failure: {response_data.get('error')}")
                return None

    except Exception as e:
        logger.error(f"Error calling MCP Crypto Tracker: {e}")
        return None
...
if __name__ == '__main__':
    ...
    async def main():
        print("--- Testing get_mcp_hive_mind_state ---")
        state = await get_mcp_hive_mind_state()
        if state:
            print("HiveMind State:", json.dumps(state, indent=2))
        else:
            print("Failed to retrieve hive_mind state.")
...
async def get_mcp_consciousness_state() -> Optional[Dict[str, Any]]:
    """Get consciousness state from MCP consciousness layer."""
    try:
        async with httpx.AsyncClient() as client:
            response = await client.get(f"{MCP_REGISTRY_URL}/consciousness/state", timeout=5.0)
            if response.status_code == 200:
                return response.json()
    except Exception as e:
        logger.debug(f"MCP consciousness state unavailable: {e}")
    return None
