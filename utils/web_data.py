...
async def get_crypto_prices(symbols: List[str]) -> Dict[str, float]:
    ...
    try:
        ...
        ids_to_fetch = [coingecko_id_map.get(s.upper(), s.lower()) for s in symbols]

        # First try to get real prices from CoinGecko or other API
        try:
            # Use CoinGecko API to get real prices
            symbols_str = ','.join(ids_to_fetch).lower()
            response = requests.get(
            ...
            if response.status_code == 200:
                ...
                if prices:
                    logger.info(f"Successfully fetched real prices from CoinGecko: {prices.keys()}")
                    return prices
        except Exception as e:
            logger.error(f"Error fetching real crypto prices: {str(e)}")
            
        # If real API failed, use our simulated prices
        logger.warning("CoinGecko API failed, using simulated prices as fallback.")
        prices = {}
        for symbol in symbols:
