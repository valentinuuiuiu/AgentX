...
class PriceFeedService:
    def __init__(self):
        ...
        self.infura_api_key = os.getenv('INFURA_API_KEY')  # Optional – Alchemy used as primary
        self.alchemy_api_key = os.getenv('ALCHEMY_API_KEY')
        # Also accept a direct RPC URL (ETHEREUM_RPC_URL) as final fallback
        self.ethereum_rpc_url = os.getenv('ETHEREUM_RPC_URL')

        if not self.etherscan_api_key:
            logger.warning("ETHERSCAN_API_KEY not set – Chainlink price feeds will be unavailable")
        if not self.infura_api_key and not self.alchemy_api_key and not self.ethereum_rpc_url:
            raise ValueError(
                "No Web3 provider configured. Set ALCHEMY_API_KEY, INFURA_API_KEY, or ETHEREUM_RPC_URL"
            )
        if not self.infura_api_key:
            logger.info("INFURA_API_KEY not set – using Alchemy / direct RPC URL for Web3 connections")
        ...
        try:
            ...
            # Final fallback: use ETHEREUM_RPC_URL directly
            if not self.w3_mainnet and self.ethereum_rpc_url:
                try:
                    self.w3_mainnet = Web3(Web3.HTTPProvider(self.ethereum_rpc_url))
                    if self.w3_mainnet.is_connected():
                        logger.info(f"✅ Connected to Ethereum Mainnet via ETHEREUM_RPC_URL")
                    else:
                        self.w3_mainnet = None
                except Exception as e:
                    logger.warning(f"Failed to initialize ETHEREUM_RPC_URL provider: {e}")
                    self.w3_mainnet = None

            if not self.w3_mainnet:
                logger.warning("⚠️  No Ethereum Mainnet Web3 connection – on-chain features degraded (prices via CryptoCompare/Binance APIs)")
