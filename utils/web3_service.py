...
class Web3Service:
    ...
    def add_provider(self, chain_id: int, rpc_url: str):
        """Add a blockchain provider with PoA support."""
        w3 = Web3(Web3.HTTPProvider(rpc_url))

        # Inject PoA middleware for networks that need it (Polygon, BSC, some testnets)
        # In web3 v7, we use ExtraDataToPOAMiddleware
        is_poa = False
        # Check by chain_id or RPC URL
        poa_chain_ids = [137, 80001, 80002, 56, 97] # Polygon, BSC main/test
        poa_keywords = ['polygon', 'bsc', 'sepolia', 'goerli', 'mumbai', 'amoy', 'bnb']

        if chain_id in poa_chain_ids:
            is_poa = True
        elif any(name in rpc_url.lower() for name in poa_keywords):
            is_poa = True

        if is_poa:
            logger.info(f"Injecting PoA middleware for chain {chain_id}")
