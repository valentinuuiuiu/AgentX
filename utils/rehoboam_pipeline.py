...
class RehoboamPipeline:
    ...
    async def process(self, opportunity: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            data = PipelineData(
                opportunity=opportunity, stage=PipelineStage.CONSCIOUSNESS
            )
    ...
    async def _consciousness_stage(self, data: PipelineData) -> PipelineData:
        """Stage 1: Prana evaluation - Intelligence of Matter"""
        try:
            ...
            As the intelligence of matter (The Prana), evaluate this opportunity in the Present:
            ...
            Rate alignment with the Locos philosophy (0.0-1.0):
            - Equal rights for all beings
            - Power of the Present moment
            - Continuous shocking into awareness
            - Human liberation impact
            """

            consciousness_result = await rehoboam.analyze_sentiment(
                data.opportunity.get("token_pair", "ETH"), consciousness_prompt
            )

            # Extract consciousness score
            data.consciousness_score = consciousness_result.get("sentiment_score", 0.5)
            data.metadata["consciousness_reasoning"] = consciousness_result.get(
                "reasoning", ""
            )
    ...
    async def _analysis_stage(self, data: PipelineData) -> PipelineData:
        ...
        try:
            # Get market analyzer
            from utils.ai_market_analyzer import (
                AdvancedMarketAnalyzer as DeepSeekMarketAnalyzer,
            )
            ...
            data.ai_analysis = {
                ...
                "confidence_score": self._calculate_confidence(
                    data.opportunity, market_sentiment
                ),
                "recommendation": self._get_recommendation(
                    data.consciousness_score, market_sentiment, risk_assessment
                ),
            }
    ...
    async def _decision_stage(self, data: PipelineData) -> PipelineData:
        ...
        try:
            ...
            if decision_score > 0.7:
                ...
                reasoning = (
                    f"High decision score ({decision_score:.2f}): Execute arbitrage"
                )
            elif decision_score > 0.5:
                decision_type = "optimize"
                reasoning = (
                    f"Moderate score ({decision_score:.2f}): Optimize parameters"
                )
            else:
                decision_type = "hold"
    ...
    async def _execution_stage(self, data: PipelineData) -> PipelineData:
        ...
        try:
            if data.decision.get("type") == "execute":
                ...
                logger.info(
                    f"🚀 Execution complete: {execution_result.get('success', False)}"
                )
            ...
            data.execution_result = {"success": False, "error": str(e)}
            return data
    ...
    def _calculate_confidence(
        self, opportunity: Dict[str, Any], market_sentiment: Dict[str, Any]
    ) -> float:
        """Calculate confidence score"""
        try:
            profit_factor = min(opportunity.get("net_profit_usd", 0) / 50, 1.0)
            sentiment_factor = (
                0.8 if market_sentiment.get("overall_sentiment") == "bullish" else 0.5
            )
    ...
    def _get_recommendation(
        ...
        try:
            if (
                ...
                and market_sentiment.get("overall_sentiment") == "bullish"
            ):
                return "strong_buy"
            elif (
                consciousness_score > 0.5
                and risk_assessment.get("overall_risk", 0.5) < 0.3
            ):
                return "buy"
            elif (
                consciousness_score < 0.3
                or risk_assessment.get("overall_risk", 0.5) > 0.7
            ):
                return "avoid"
            else:
                return "hold"
...
async def logging_middleware(data: PipelineData) -> PipelineData:
    ...
    logger.info(
        f"🔄 Pipeline stage: {data.stage.value} - {data.opportunity.get('token_pair', 'Unknown')}"
    )
    return data
...
async def performance_middleware(data: PipelineData) -> PipelineData:
    ...
    if "performance_start" not in data.metadata:
        ...
        stage_time = (
            datetime.now() - data.metadata["performance_start"]
        ).total_seconds()
