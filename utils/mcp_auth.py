...
# ⚠️ VSR HARDENING: No hardcoded secrets — environment must provide them
IS_TESTING = os.getenv('TESTING') == 'true'

JWT_SECRET = os.getenv('JWT_SECRET')
if not JWT_SECRET or len(JWT_SECRET) < 32:
    if IS_TESTING:
        JWT_SECRET = "dummy_jwt_secret_for_testing_purposes_only_1234567890"
    else:
        raise RuntimeError(
...
if not MCP_TOKEN or len(MCP_TOKEN) < 32:
    if IS_TESTING:
        MCP_TOKEN = "dummy_mcp_token_for_testing_purposes_only_1234567890"
    else:
        raise RuntimeError(
            "CRITICAL: MCP_TOKEN environment variable not set or too short (< 32 bytes). "
            "Generate with: openssl rand -base64 48"
        )
...
if not API_TOKEN or len(API_TOKEN) < 32:
    if IS_TESTING:
        API_TOKEN = "dummy_api_token_for_testing_purposes_only_1234567890"
    else:
        raise RuntimeError(
            "CRITICAL: API_TOKEN environment variable not set or too short (< 32 bytes). "
            "Generate with: openssl rand -base64 48"
        )
