...
MOCK_SERVICE_NAME_FOUND = "hive_mind-layer"
MOCK_SERVICE_URL_FOUND = "http://hive_mind-layer-service:3005"
MOCK_SERVICE_NAME_ALIAS = "mcp-hive_mind-layer" # an alias that should also be found

MOCK_REGISTRY_DATA_VALID = {
    "services": {
        MOCK_SERVICE_NAME_FOUND: {
            "url": MOCK_SERVICE_URL_FOUND,
            "description": "Provides hive_mind state.",
...
async def test_get_service_url_multiple_names_to_find_first_one_exists():
    ...
    service_url = await _get_service_url_from_registry(
        mock_client,
        [MOCK_SERVICE_NAME_FOUND, "market-analyzer", "nonexistent-service"], # MOCK_SERVICE_NAME_FOUND is "hive_mind-layer"
        "TestContextMultipleFindFirst"
    )
    assert service_url == MOCK_SERVICE_URL_FOUND # Correct, "hive_mind-layer" is first and found
...
async def test_get_service_url_multiple_names_to_find_second_one_exists():
    ...
    # Expected: "market-analyzer" URL because it's found before MOCK_SERVICE_NAME_FOUND ("hive_mind-layer") in the search list.
...
# Test data for hive_mind state
MOCK_HIVE_MIND_STATE_DATA = {"awareness_level": 0.9, "cognitive_load": 0.3}
MOCK_HIVE_MIND_SERVICE_URL = "http://fake-hive_mind-service:1234"
...
# === Tests for get_mcp_hive_mind_state ===

@pytest.mark.asyncio
@patch('utils.mcp_clients._get_service_url_from_registry', new_callable=AsyncMock)
async def test_get_mcp_hive_mind_state_success(mock_get_url, caplog):
    import logging
    caplog.set_level(logging.INFO)
    mock_get_url.return_value = MOCK_HIVE_MIND_SERVICE_URL

    mock_response_data = MOCK_HIVE_MIND_STATE_DATA

    async def mock_client_get(url, **kwargs):
        assert url == f"{MOCK_HIVE_MIND_SERVICE_URL}/state"
    ...
    with patch('httpx.AsyncClient.get', side_effect=mock_client_get):
        ...
        from utils.mcp_clients import get_mcp_hive_mind_state
        result = await get_mcp_hive_mind_state()

    assert result == mock_response_data
    mock_get_url.assert_called_once()
    assert "Successfully fetched hive_mind state." in caplog.text

@pytest.mark.asyncio
@patch('utils.mcp_clients._get_service_url_from_registry', new_callable=AsyncMock)
async def test_get_mcp_hive_mind_state_service_url_not_found(mock_get_url, caplog):
    import logging
    ...
    # The calling function get_mcp_hive_mind_state itself doesn't log when service_url is None,
...
@pytest.mark.asyncio
@patch('utils.mcp_clients._get_service_url_from_registry', new_callable=AsyncMock)
async def test_get_mcp_hive_mind_state_service_call_http_error(mock_get_url, caplog):
    import logging
    ...
    assert result is None
    mock_actual_get.assert_called_once_with(f"{MOCK_HIVE_MIND_SERVICE_URL}/state")
    assert "HTTP status error 500 for MCP HiveMind Layer (State): Internal Server Error" in caplog.text

@pytest.mark.asyncio
@patch('utils.mcp_clients._get_service_url_from_registry', new_callable=AsyncMock)
async def test_get_mcp_hive_mind_state_service_call_network_error(mock_get_url, caplog):
    import logging
    caplog.set_level(logging.ERROR)
    mock_get_url.return_value = MOCK_HIVE_MIND_SERVICE_URL
    ...
    assert result is None
    mock_actual_get.assert_called_once_with(f"{MOCK_HIVE_MIND_SERVICE_URL}/state")
    assert "HTTP request error for MCP HiveMind Layer (State): Network down" in caplog.text

@pytest.mark.asyncio
@patch('utils.mcp_clients._get_service_url_from_registry', new_callable=AsyncMock)
async def test_get_mcp_hive_mind_state_invalid_json(mock_get_url, caplog):
    import logging
    caplog.set_level(logging.ERROR)
    mock_get_url.return_value = MOCK_HIVE_MIND_SERVICE_URL
    ...
    assert result is None
    mock_actual_get.assert_called_once_with(f"{MOCK_HIVE_MIND_SERVICE_URL}/state")
    assert "Error decoding JSON response from MCP HiveMind Layer (State)" in caplog.text

@pytest.mark.asyncio
@patch('utils.mcp_clients._get_service_url_from_registry', new_callable=AsyncMock)
async def test_get_mcp_hive_mind_state_unexpected_data_type(mock_get_url, caplog):
    import logging
    ...
    assert f"Unexpected data format from MCP HiveMind Layer (State) at {MOCK_HIVE_MIND_SERVICE_URL}/state. Expected dict, got <class 'list'>" in caplog.text
...
async def test_get_mcp_market_analysis_service_url_not_found(mock_get_url, caplog):
    ...
    assert result is None
    # Similar to the hive_mind state test, the main function doesn't log here.
