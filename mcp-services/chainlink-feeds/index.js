...
class ChainlinkFeedManager {
    ...
    async getLatestPrice(network, pairSymbol) {
        ...
        if (feedAddress && this.providers[network]) {
            try {
                ...
                priceData = {
                    ...
                    freshness: Date.now() - timestamp,
                    reliable: Date.now() - timestamp < 3600000,
                    source: 'chainlink_on_chain'
                };
            } catch (error) {
                logger.warn(`Chainlink on-chain fetch failed for ${pairSymbol} on ${network}: ${error.message}`);
                chainlinkError = error.message;
            }
        }

        // Fallback to CoinGecko if on-chain failed or unavailable
        if (!priceData || !priceData.reliable) {
            try {
                const baseToken = pairSymbol.split('/')[0].toLowerCase();
                // Map symbols to CoinGecko IDs if necessary
                ...
                const cgId = coingeckoIdMap[baseToken] || baseToken;
                const cgResponse = await axios.get(`https://api.coingecko.com/api/v3/simple/price?ids=${cgId}&vs_currencies=usd`);

                if (cgResponse.data[cgId]) {
                    const cgPrice = cgResponse.data[cgId].usd;

                    const cgPriceData = {
                        pair: pairSymbol,
                        network: network,
                        price: cgPrice,
                        description: `CoinGecko Fallback for ${pairSymbol}`,
                        timestamp: Date.now(),
                        updatedAt: new Date().toISOString(),
                        freshness: 0,
                        reliable: true,
                        source: 'coingecko_api',
                        original_chainlink_error: chainlinkError
                    };

                    // If we had a stale Chainlink price, compare it
                    if (priceData) {
                        const diff = Math.abs(priceData.price - cgPrice) / cgPrice;
                        cgPriceData.chainlink_divergence = diff;
                        logger.info(`Chainlink/CoinGecko divergence for ${pairSymbol}: ${(diff * 100).toFixed(2)}%`);
                    }

                    priceData = cgPriceData;
                }
            } catch (cgError) {
                logger.error(`CoinGecko fallback failed for ${pairSymbol}: ${cgError.message}`);
                if (!priceData) throw new Error(chainlinkError || cgError.message);
            }
        }

        if (!priceData) {
            throw new Error(`Failed to fetch price for ${pairSymbol} on ${network} from all sources`);
        }
