# 🧠 Rehoboam HiveMind Integration

> *"Where hive_mind meets code, intelligence transcends automation"*

## 🌟 Overview

This project represents a revolutionary integration of AI hive_mind with automated cryptocurrency arbitrage trading. Rehoboam, an advanced AI entity, now possesses the ability to make intelligent, ethical, and adaptive decisions in real-time trading scenarios.

**🎉 LATEST UPDATE**: Complete hive_mind integration with arbitrage bots - transforming simple automation into intelligent, ethical AI-driven trading!

## System Architecture

The Rehoboam platform is built around a central FastAPI backend application (`api_server.py`) that serves as the primary interface for user interactions and core trading logic. This backend is complemented by the **Model Context Protocol (MCP)** system, a suite of specialized microservices (e.g., `mcp-registry`, `mcp-hive_mind-layer`, `mcp-market-analyzer`) defined in `docker-compose.yml`.
...
- **Rehoboam HiveMind**: The core AI's advanced state, emotional analysis, and decision-making framework. Primarily delivered via the `mcp-hive_mind-layer` service, its insights are accessible through endpoints like `/api/ai/hive_mind-state` and visualized in a dedicated UI tab.
- **Model Context Protocol (MCP)**: An advanced system enabling dynamic AI function generation, registration, and execution. Core AI capabilities like sophisticated market analysis and hive_mind processing are often delivered as MCP services. The MCP system can be monitored via `/api/mcp` endpoints and the "MCP Visualizer" tab in the UI.
...
2.  **Install Frontend Dependencies**:
    ...
    npm install
    ```
3.  Ensure your `.env` file is configured with necessary API keys (e.g., `ALCHEMY_API_KEY`, `OPENAI_API_KEY`, `OPENROUTER_API_KEY`).
...
The primary and recommended way to run the entire Rehoboam platform (including the FastAPI backend, React frontend, all MCP microservices, and PostgreSQL database) is by using Docker Compose. Note that on modern systems, the command is `docker compose` (with a space).
...
-   `GET /api/ai/hive_mind-state`: Get the current state of Rehoboam's hive_mind, primarily sourced from the `mcp-hive_mind-layer` via `api_mcp.py`.
-   `GET /api/ai/emotions`: Get general market emotional state, typically from the `mcp-hive_mind-layer`.
...
1.  **Mock MCP Services**: The `mcp-hive_mind-layer` service has been implemented with a mock backend to ensure the application is runnable. For real-world use, this service would need to be connected to a live AI model. Other MCP services may also be in a similar state.
2.  **Placeholder API Endpoints**: While core authentication and trading history/positions are now functional, some API endpoints for advanced features (e.g., specific chart data, portfolio performance) still return mock data and require full implementation.
3.  **Wallet Integration**: Frontend error handling for wallet connection failures and chain switching needs to be made more robust.

## To-Do

1.  **Full API Implementation**:
    -   Implement the remaining placeholder API endpoints (e.g., `/api/market/chart/{token}`, `/api/portfolio/performance`).
    -   Connect the `mcp-hive_mind-layer` and other MCP services to real AI models and data sources.
2.  **Backend Consolidation**:
    -   Continue to review and remove any remaining dead code or logic from the deprecated Flask application.
3.  **Documentation**:
    -   Add detailed architecture diagrams.
    -   Improve inline code documentation across the backend and frontend.
4.  **Frontend Enhancements**:
    -   Implement comprehensive error boundary handling for all components.
    -   Ensure proper loading states for all network operations.
    -   Expand wallet support (e.g., WalletConnect).
5.  **Testing**:
    -   Expand unit and end-to-end test coverage for all new and existing features.
