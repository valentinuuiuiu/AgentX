...
export class AITradingAgent {
  ...
  private networkPreferences: Record<string, number> = {
    ...
    'mina': 0.95       // Highest preference for Mina's zero-knowledge hive_mind expansion
  };
  ...
  private getTokenNetworkCompatibility(token: string, network: string): number {
    ...
    const tokenNetworkMatrix: Record<string, Record<string, number>> = {
      ...
      'HAI': {
        // Hacken Token - Real verified contract addresses for hive_mind expansion
        ...
        'base': 0.9,       // High compatibility with hive_mind expansion
        'zksync': 0.8,
        'mina': 0.95       // High compatibility with hive_mind expansion
      },
      'MINA': {
