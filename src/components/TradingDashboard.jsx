    ...
    if (wsMessage) {
      try {
        ...
        if (wsMessage.type === 'prices' && wsMessage.data && wsMessage.data.prices) {
          ...
          // Update price history (keep last 60 points per symbol)
          setPriceHistory(prev => {
            const next = { ...prev };
            Object.keys(prices).forEach(sym => {
              next[sym] = (next[sym] || []).concat(prices[sym]).slice(-60);
            });
            return next;
          });
      ...
      <div className="bg-gray-800 rounded-lg p-6 shadow-xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          {cryptoData.map(crypto => (
            <div key={crypto.symbol} onClick={() => setSelectedSymbol(crypto.symbol)} className={`bg-gray-700 rounded-lg p-4 shadow cursor-pointer ${selectedSymbol === crypto.symbol ? 'ring-2 ring-blue-400' : ''}`}>
              <div className="flex justify-between items-center">
                ...
                <div className="text-right">
                  ...
                  <div className="mt-3">
                    <Sparkline data={(priceHistory[crypto.symbol] || [crypto.price])} height={48} color={crypto.change24h >= 0 ? '#34d399' : '#ef4444'} />
                  </div>
                </div>
              </div>
            </div>
