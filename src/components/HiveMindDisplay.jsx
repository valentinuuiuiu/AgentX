...
const HiveMindDisplay = () => {
  const [hive_mindData, setHiveMindData] = useState(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(null);
      try {
        const response = await fetch('/api/ai/hive_mind-state');
        if (!response.ok) {
        ...
        console.error("Failed to fetch hive_mind state:", e);
        setError(e.message);
      } finally {
        setIsLoading(false);
      }
  ...
  if (isLoading) {
    return (
      <div className="flex justify-center items-center h-64">
        <p className="text-xl text-gray-400">Loading HiveMind State...</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <Card className="bg-red-900/20 border-red-700">
        <CardHeader>
          <CardTitle className="text-red-400">Error Fetching HiveMind State</CardTitle>
        </CardHeader>
        <CardContent>
  ...
  if (!hive_mindData) {
    return <p className="text-center text-gray-500 italic">No hive_mind data available.</p>;
  }

  const { source, timestamp, mcp_data, local_rehoboam_info } = hive_mindData;
  ...
  return (
    <div className="space-y-6 p-4 bg-gray-800 rounded-lg shadow-xl">
      <CardHeader className="p-0 mb-4">
        <CardTitle className="text-3xl font-bold text-blue-300">AI HiveMind State</CardTitle>
        <CardDescription className="text-gray-400">
          Current operational state of the AI hive_mind layers. Timestamp: {new Date(timestamp).toLocaleString()}
        </CardDescription>
      </CardHeader>
      ...
      <Card>
        ...
        <CardContent>
          <Badge variant={source === 'mcp_hive_mind_layer' ? 'default' : 'secondary'} className="text-lg">
            {source || naText}
          </Badge>
        </CardContent>
      </Card>
      ...
      {mcp_data && (
        <Card>
          <CardHeader>
            <CardTitle className="text-xl text-blue-400">MCP HiveMind Metrics</CardTitle>
          </CardHeader>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <p className="text-sm text-gray-300">
              HiveMind Matrix Available (Local):
              <Badge variant={local_rehoboam_info.hive_mind_matrix_available ? 'default' : 'outline'} className="ml-2">
                {local_rehoboam_info.hive_mind_matrix_available ? 'Yes' : 'No'}
              </Badge>
            </p>
            <div>
...
export default HiveMindDisplay;
