    ...
    <div className="space-y-6">
      ...
      {systemHealth && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <Card>
            <CardContent className="p-4">
              <div className="flex justify-between items-center mb-2">
                <h3 className="font-semibold">🧠 HiveMind</h3>
                <Badge className={getStatusColor(systemHealth.hive_mind_layer.status)}>
                  {systemHealth.hive_mind_layer.status}
                </Badge>
              </div>
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span>Awareness:</span>
                  <span>{(systemHealth.hive_mind_layer.awareness_level * 100).toFixed(0)}%</span>
                </div>
                <Progress value={systemHealth.hive_mind_layer.awareness_level * 100} className="h-2" />
                <div>Liberation: {(systemHealth.hive_mind_layer.liberation_progress * 100).toFixed(1)}%</div>
              </div>
            </CardContent>
          </Card>
      ...
      {systemMetrics && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <Card>
            <CardContent className="p-4 text-center">
              <div className="text-2xl font-bold text-purple-600">{systemMetrics.hive_mind_score}%</div>
              <div className="text-sm text-gray-600">HiveMind Score</div>
            </CardContent>
          </Card>
        </div>
      )}
